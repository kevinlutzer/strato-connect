...
int8_t RF4463::getRSSI(){
  uint8_t buf_len = 6;
  uint8_t buf[buf_len];

  getCommand(6, RF4463_CMD_GET_MODEM_STATUS, buf);		// read part info to check if 4463 works
  return buf[3];
  uint8_t half_buf = buf[3] / 2;
  int8_t rssi = half_buf - RF4463_RSSI_CAL_CONSTANT;
  return rssi;
}
